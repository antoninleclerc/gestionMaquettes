...
namespace GestionMaquettes
{
	...
	{	
		...
		{
			ost<<std::setw(15)<<std::left<<d_UE[0]->code()<<std::setw(10)<<std::left<<d_UE[0]->coefficient()<<std::setw(5)<<std::left<<ECTS()<<std::setw(40)<<std::left<<"UE "+d_UE[0]->intitule()<<std::setw(10)<<std::left<<" "<<std::setw(10)<<std::left<<" "<<std::setw(10)<<std::left<<" "<<std::setw(10)<<std::left<<totalNombreHeures()<<std::endl;
			for(int i=1; i<=nombreECUE(); i++)
			d_UE[i]->affiche(ost);
		}
		else //si UE seule
		{
			ost<<std::setw(15)<<std::left<<d_UE[0]->code()<<std::setw(10)<<std::left<<d_UE[0]->coefficient()<<std::setw(5)<<std::left<<ECTS()<<std::setw(40)<<std::left<<"UE " + d_UE[0]->intitule()<<std::setw(10)<<std::left<<totalHeuresCM()<<std::setw(10)<<std::left<<totalHeuresTD()<<std::setw(10)<<std::left<<totalHeuresTP()<<std::setw(10)<<std::left<<totalNombreHeures()<<std::endl;
		}		
	}
	...
	void UE::enregistrerUE(const std::string &mention) 
	{
		...
		if(fichier){
			if(nombreECUE()>0) //si UE composé de au moins un ECUE
			{
				fichier << std::setw(10) << std::left << d_UE[0]->code() << std::setw(12) << std::left << d_UE[0]->coefficient() << std::setw(5) << std::left << ECTS() << std::setw(45) << std::left << "UE " + d_UE[0]->intitule() <<std::setw(10)<<std::left<<" "<<std::setw(10)<<std::left<<" "<<std::setw(10)<<std::left<<" "<<std::setw(10)<<std::left<<totalNombreHeures() << std::endl;
				for(int i = 1; i <= nombreECUE(); i++)
